function VARbs = doPVAR2(VAR,DAT,nboot,clevel)
  ...
  jj=1;
     while jj<nboot+1
       ...
       varsb(1:VAR.p,:)=VAR.vars(1:VAR.p,:);
        ...
        VARBS = VAR;
        VARBS.vars = varsb;
        VARBS.taxshocks = [VAR.TAXSHOCKS(1:VAR.p,:); VAR.TAXSHOCKS.*(rr*ones(1,VAR.k))];
        VARBS = doPVAR2(VARBS);
        ...
        VARbs.BS_taxshocks(:,jj) = VARBS.taxshocks;
        truetaxshocks = [VAR.TAXSHOCKS(1:VAR.p,:); VAR.SigmaT*(VAR.D(1,1)*100*VAR.et.*rr/DAT.TRY) ];

        truetaxshocks(VARBS.taxshocks==0)=0;
        VARbs.BS_taxshocks_true(:,jj) = truetaxshocks;          
